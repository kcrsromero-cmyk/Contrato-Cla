    ...
    <div className="space-y-6 animate-fade-in" id="supervisores-section">
      ...
      {selectedDetailContract && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-xs animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-3xl rounded-2xl max-h-[90vh] overflow-y-auto flex flex-col shadow-2xl relative animate-scale-up" id="supervisor-contract-detail-modal">
            ...
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between bg-slate-50 dark:bg-slate-950 sticky top-0 z-10">
              <div className="min-w-0 pr-6">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 truncate font-sans" title={selectedDetailContract.proveedor_adjudicado}>
                  {normalizeName(selectedDetailContract.proveedor_adjudicado)}
                </h3>
                <p className="text-xs font-mono text-slate-500 dark:text-slate-450 mt-0.5">
                  ID: {selectedDetailContract.id_contrato} | {selectedDetailContract.tipodocproveedor || 'CC/NIT'}: {maskDocument(selectedDetailContract.documento_proveedor || '', selectedDetailContract.tipodocproveedor)} | Referencia: {selectedDetailContract.referencia_del_contrato || 'No reportada'}
                </p>
                <div className="mt-2 flex items-center gap-1.5">
                  ...
                  {renderEstadoSemaforo(selectedDetailContract.estado_contrato || '')}
                </div>
              </div>
              <button 
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-1.5">
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-line shadow-2xs">
                  {selectedDetailContract.objeto_del_contrato}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-950/20 shadow-2xs">
                  ...
                  <div className="text-xs font-bold text-slate-800 dark:text-slate-200">{formatDate(selectedDetailContract.fecha_de_firma)}</div>
                </div>
                <div className="p-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-950/20 shadow-2xs">
                  <div className="text-[10px] font-bold font-mono uppercase tracking-widest text-slate-400 dark:text-slate-500 flex items-center gap-1 mb-1">
                    <Calendar className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-450" /> Inicio de Contrato
                  </div>
                  <div className="text-xs font-bold text-slate-800 dark:text-slate-200">{formatDate(selectedDetailContract.fecha_de_inicio_del_contrato)}</div>
                </div>
                <div className="p-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-950/20 shadow-2xs">
                  <div className="text-[10px] font-bold font-mono uppercase tracking-widest text-slate-400 dark:text-slate-500 flex items-center gap-1 mb-1">
                    <Calendar className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-450" /> Fin de Contrato
                  </div>
                  <div className="text-xs font-bold text-slate-800 dark:text-slate-200">{formatDate(selectedDetailContract.fecha_de_fin_del_contrato)}</div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-bold font-mono uppercase tracking-widest text-slate-400 dark:text-slate-500 flex items-center gap-1.5">
                  <DollarSign className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" /> Balance Financiero Reportado
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="p-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 shadow-2xs">
                    ...
                    <div className="text-sm font-bold text-slate-900 dark:text-slate-100 mt-1">
                      {formatCOP(selectedDetailContract.valor_del_contrato)}
                    </div>
                  </div>
                  <div className="p-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 shadow-2xs">
                    ...
                    <div className="text-sm font-bold text-emerald-700 dark:text-emerald-400 mt-1">
                      {formatCOP(selectedDetailContract.valor_pagado)}
                    </div>
                  </div>
                  <div className="p-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 shadow-2xs">
                    ...
                    <div className="text-sm font-bold text-amber-700 dark:text-amber-450 mt-1">
                      {formatCOP(selectedDetailContract.valor_pendiente_de_pago)}
                    </div>
                  </div>
                  <div className="p-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 shadow-2xs">
                    ...
                    <div className="text-sm font-bold text-slate-600 dark:text-slate-400 mt-1">
                      {formatCOP(selectedDetailContract.valor_pendiente_de_ejecucion)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 border border-slate-200 dark:border-slate-800 rounded-xl space-y-2 bg-white dark:bg-slate-900 shadow-2xs">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-xs font-bold text-slate-800 dark:text-slate-200 leading-snug">{selectedDetailContract.modalidad_de_contratacion}</div>
                  </div>
                  {selectedDetailContract.justificacion_modalidad_de && (
                    <div className="space-y-1 pt-1.5 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <div className="text-xs font-medium text-slate-600 dark:text-slate-400 leading-snug">{selectedDetailContract.justificacion_modalidad_de}</div>
                    </div>
                  )}
                </div>
                ...
                <div className="p-4 border border-slate-200 dark:border-slate-800 rounded-xl space-y-2 bg-white dark:bg-slate-900 shadow-2xs">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-xs font-bold text-slate-800 dark:text-slate-200">{normalizeName(selectedDetailContract.nombre_supervisor)}</div>
                  </div>
                  {selectedDetailContract.nombre_ordenador_del_gasto && (
                    <div className="space-y-1 pt-1.5 border-t border-slate-100 dark:border-slate-800">
                      <div className="text-[10px] font-mono uppercase text-slate-400 dark:text-slate-500">Ordenador del Gasto:</div>
                      <div className="text-xs font-medium text-slate-600 dark:text-slate-400">{normalizeName(selectedDetailContract.nombre_ordenador_del_gasto)}</div>
                    </div>
                  )}
                </div>
              </div>

              {/* Clasificación y Estructura de la Entidad (Campos SECOP II) */}
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl space-y-3 shadow-2xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                  <div>
                    ...
                    <div className="font-bold text-slate-800 dark:text-slate-200 mt-0.5">{selectedDetailContract.orden || 'No especificado'}</div>
                  </div>
                  <div>
                    ...
                    <div className="font-bold text-slate-800 dark:text-slate-200 mt-0.5">{selectedDetailContract.entidad_centralizada || 'No especificado'}</div>
                  </div>
                  <div>
                    <div className="text-slate-400 dark:text-slate-500 font-bold font-mono text-[9px] uppercase tracking-wider">Duración Original</div>
                    <div className="font-bold text-slate-800 dark:text-slate-200 mt-0.5">{selectedDetailContract.duraci_n_del_contrato} días</div>
                  </div>
                </div>
              </div>
            ...
            {/* Footer containing link to SECOP II */}
            {getSecopUrl(selectedDetailContract) && (
              <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex items-center justify-end sticky bottom-0 z-10 rounded-b-2xl">
                <a
                  href={getSecopUrl(selectedDetailContract)!}
                  target="_blank"
                  referrerPolicy="no-referrer"
                  rel="noopener noreferrer"
                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-bold text-xs rounded-xl shadow-xs hover:shadow-md transition-all flex items-center gap-1.5 cursor-pointer"
                >
                  <ExternalLink className="w-4 h-4" /> Ver Expediente en SECOP II
                </a>
              </div>
            )}
